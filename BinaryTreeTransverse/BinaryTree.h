<<<<<<< HEAD
//////////////////////////////////////////////////////////////////////////
// Author:	XYZ
// Date:	2016-09-19
// BinaryTree.h: ¶þ²æÊ÷µÄÊµÏÖ
// °üº¬ÁË¶ÁÈ¡ÌØ¶¨¸ñÊ½µÄÎÄµµÀ´½¨Á¢¶þ²æÊ÷
// ¸ñÊ½ÎªÃ¿Ò»ÐÐ3¸ö×Ö·û´®±»","·Ö¿ª£¬ÒÀ´Î´ú±í¸ù¡¢×ó×Ó¡¢ÓÒ×Ó½Úµã
// ÒÔ¼°ÊµÏÖÁËÏÈÐò¡¢ÖÐÐò¡¢ºóÐò±éÀú
// Í¨¹ýÖÐÐò±éÀúÊµÏÖÊ÷½ÚµãµÄÊÍ·Å
// ½áµãÀàÐÍÎª×Ö·û´®£¨´ÓÎÄµµÖÐ¶ÁÈ¡£©
// Ê÷µÄÊý¾Ý½á¹¹ÎªBinaryTree
// ²âÊÔº¯ÊýÎªvoid use_binary_tree();
// Êý¾ÝÎÄµµÎªdata.txt
//////////////////////////////////////////////////////////////////////////
#ifndef _BINARY_TREE
#define _BINARY_TREE
#include "SimpleSplit.h"
#include <iostream>
#include <vector>
#include <string>

// Êý½áµã£¬ÔªËØÀàÐÍÎª×Ö·û´®
struct TreeNode
{
	TreeNode(const std::string& _data, TreeNode* _left, TreeNode* _right)
		: data(_data), lchild(_left), rchild(_right) { }
	std::string data;
	TreeNode* lchild;
	TreeNode* rchild;
	~TreeNode() { std::cout << "Îö¹¹" << data << std::endl; }
};

struct TreeNodePrinter
{
	TreeNodePrinter(std::ostream& os)
		: _os(os) { }
	void operator()(TreeNode* p)
	{
		if (!p->data.empty())
			_os << p->data << " ";
	}
	std::ostream& _os;
};

struct BinaryTree
{
public:
	typedef TreeNode Node;
	typedef Node* NodePtr;

	BinaryTree() = default;
	BinaryTree(const std::string& filename) { createFromFile(filename); }
	~BinaryTree() { PreOrderTraverse([](NodePtr p) { if (p) delete p; }); }

	// ¶ÁÈ¡Ö¸¶¨¸ñÊ½ÎÄ¼þ£¬²¢½¨Á¢ÏàÓ¦µÄ¶þ²æÊ÷
	void createFromFile(const std::string& filename);

	// ÏÈÐò±éÀú£¬¸ù->×ó×Ó->ÓÒ×Ó
	template <typename Pred>
	void PreOrderTraverse(Pred _Pr)
	{
		__PreOrderTraverse(root, _Pr);
	}

	// ÖÐÐò±éÀú£¬×ó×Ó->¸ù->ÓÒ×Ó
	template <typename Pred>
	void InOrderTraverse(Pred _Pr)
	{
		__InOrderTraverse(root, _Pr);
	}

	// ºóÐò±éÀú£¬×ó×Ó->ÓÒ×Ó->¸ù
	template <typename Pred>
	void PostOrderTraverse(Pred _Pr)
	{
		__PostOrderTraverse(root, _Pr);
	}

private:
	NodePtr root = nullptr;  // ¸ù½Úµã	
};

// ²âÊÔÓÃÀý
inline void use_binary_tree()
{
	BinaryTree tree;
	tree.createFromFile("data.txt");
	tree.PreOrderTraverse(TreeNodePrinter(std::cout));
	printf("\n");
	tree.InOrderTraverse(TreeNodePrinter(std::cout));
	printf("\n");
	tree.PostOrderTraverse(TreeNodePrinter(std::cout));
	printf("\n");
}

template <typename Pred>
void __PreOrderTraverse(TreeNode* curr, Pred _Pr)
{	// ÏÈÐò±éÀúµÄ¾ßÌåÊµÏÖ
	if (curr)
	{
		TreeNode* lchild = curr->lchild;
		TreeNode* rchild = curr->rchild;
		_Pr(curr);
		__PreOrderTraverse(lchild, _Pr);
		__PreOrderTraverse(rchild, _Pr);
	}
}

template <typename Pred>
void __InOrderTraverse(TreeNode* curr, Pred _Pr)
{	// ÖÐÐò±éÀúµÄ¾ßÌåÊµÏÖ
	if (curr)
	{
		TreeNode* lchild = curr->lchild;
		TreeNode* rchild = curr->rchild;
		__InOrderTraverse(lchild, _Pr);
		_Pr(curr);
		__InOrderTraverse(rchild, _Pr);
	}
}

template <typename Pred>
void __PostOrderTraverse(TreeNode* curr, Pred _Pr)
{	// ºóÐò±éÀúµÄ¾ßÌåÊµÏÖ
	if (curr)
	{
		TreeNode* lchild = curr->lchild;
		TreeNode* rchild = curr->rchild;
		__PostOrderTraverse(lchild, _Pr);
		__PostOrderTraverse(rchild, _Pr);
		_Pr(curr);
	}
}

#endif
=======
//////////////////////////////////////////////////////////////////////////
// Author:	XYZ
// Date:	2016-09-19
// BinaryTree.h: äºŒå‰æ ‘çš„å®žçŽ°
// åŒ…å«äº†è¯»å–ç‰¹å®šæ ¼å¼çš„æ–‡æ¡£æ¥å»ºç«‹äºŒå‰æ ‘
// æ ¼å¼ä¸ºæ¯ä¸€è¡?ä¸ªå­—ç¬¦ä¸²è¢?,"åˆ†å¼€ï¼Œä¾æ¬¡ä»£è¡¨æ ¹ã€å·¦å­ã€å³å­èŠ‚ç‚?// ä»¥åŠå®žçŽ°äº†å…ˆåºã€ä¸­åºã€åŽåºéåŽ?// é€šè¿‡ä¸­åºéåŽ†å®žçŽ°æ ‘èŠ‚ç‚¹çš„é‡Šæ”¾
// ç»“ç‚¹ç±»åž‹ä¸ºå­—ç¬¦ä¸²ï¼ˆä»Žæ–‡æ¡£ä¸­è¯»å–ï¼‰
// æ ‘çš„æ•°æ®ç»“æž„ä¸ºBinaryTree
// æµ‹è¯•å‡½æ•°ä¸ºvoid use_binary_tree();
// æ•°æ®æ–‡æ¡£ä¸ºdata.txt
//////////////////////////////////////////////////////////////////////////
#ifndef _BINARY_TREE
#define _BINARY_TREE
#include "SimpleSplit.h"
#include <iostream>
#include <vector>
#include <string>

// æ•°ç»“ç‚¹ï¼Œå…ƒç´ ç±»åž‹ä¸ºå­—ç¬¦ä¸²
struct TreeNode
{
	TreeNode(const std::string& _data, TreeNode* _left, TreeNode* _right)
		: data(_data), lchild(_left), rchild(_right) { }
	std::string data;
	TreeNode* lchild;
	TreeNode* rchild;
	~TreeNode() { std::cout << "æžæž„" << data << std::endl; }
};

struct TreeNodePrinter
{
	TreeNodePrinter(std::ostream& os)
		: _os(os) { }
	void operator()(TreeNode* p)
	{
		if (!p->data.empty())
			_os << p->data << " ";
	}
	std::ostream& _os;
};

struct BinaryTree
{
public:
	typedef TreeNode Node;
	typedef Node* NodePtr;

	BinaryTree() = default;
	BinaryTree(const std::string& filename) { createFromFile(filename); }
	~BinaryTree() { PreOrderTraverse([](NodePtr p) { if (p) delete p; }); }

	// è¯»å–æŒ‡å®šæ ¼å¼æ–‡ä»¶ï¼Œå¹¶å»ºç«‹ç›¸åº”çš„äºŒå‰æ ‘
	void createFromFile(const std::string& filename);

	// å…ˆåºéåŽ†ï¼Œæ ¹->å·¦å­->å³å­
	template <typename Pred>
	void PreOrderTraverse(Pred _Pr)
	{
		__PreOrderTraverse(root, _Pr);
	}

	// ä¸­åºéåŽ†ï¼Œå·¦å­?>æ ?>å³å­
	template <typename Pred>
	void InOrderTraverse(Pred _Pr)
	{
		__InOrderTraverse(root, _Pr);
	}

	// åŽåºéåŽ†ï¼Œå·¦å­?>å³å­->æ ?	template <typename Pred>
	void PostOrderTraverse(Pred _Pr)
	{
		__PostOrderTraverse(root, _Pr);
	}

private:
	NodePtr root = nullptr;  // æ ¹èŠ‚ç‚?
};

// æµ‹è¯•ç”¨ä¾‹
inline void use_binary_tree()
{
	BinaryTree tree;
	tree.createFromFile("data.txt");
	tree.PreOrderTraverse(TreeNodePrinter(std::cout));
	printf("\n");
	tree.InOrderTraverse(TreeNodePrinter(std::cout));
	printf("\n");
	tree.PostOrderTraverse(TreeNodePrinter(std::cout));
	printf("\n");
}

template <typename Pred>
void __PreOrderTraverse(TreeNode* curr, Pred _Pr)
{	// å…ˆåºéåŽ†çš„å…·ä½“å®žçŽ?	if (curr)
	{
		TreeNode* lchild = curr->lchild;
		TreeNode* rchild = curr->rchild;
		_Pr(curr);
		if (lchild)
			__PreOrderTraverse(lchild, _Pr);
		if (rchild)
			__PreOrderTraverse(rchild, _Pr);
	}
}

template <typename Pred>
void __InOrderTraverse(TreeNode* curr, Pred _Pr)
{	// ä¸­åºéåŽ†çš„å…·ä½“å®žçŽ?	if (curr)
	{
		TreeNode* lchild = curr->lchild;
		TreeNode* rchild = curr->rchild;
		if (lchild)
			__InOrderTraverse(lchild, _Pr);
		_Pr(curr);
		if (rchild)
			__InOrderTraverse(rchild, _Pr);
	}
}

template <typename Pred>
void __PostOrderTraverse(TreeNode* curr, Pred _Pr)
{	// åŽåºéåŽ†çš„å…·ä½“å®žçŽ?	if (curr)
	{
		TreeNode* lchild = curr->lchild;
		TreeNode* rchild = curr->rchild;
		if (lchild)
			__PostOrderTraverse(lchild, _Pr);
		if (rchild)
			__PostOrderTraverse(rchild, _Pr);
		_Pr(curr);
	}
}

#endif
>>>>>>> origin/master
